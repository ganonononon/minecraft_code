...
└── cogs/              # 機能ごとに分割したモジュール
    ...
    └── backup.py      # EBSスナップショットによる手動バックアップ
```
...
- `cogs.backup`（バックアップ）を読み込む
- `cogs.death`（死亡通知）を読み込む
...
### `cogs/backup.py` — EBSバックアップ

Discordのスラッシュコマンドでいつでも手動バックアップを実行できます。

| コマンド | 説明 |
|----------|------|
| `/backup` | EBSスナップショットを作成する |

- EC2の起動・停止に関わらず実行可能
- 作成したSnapshot IDをDiscordに通知
...
| `GUILD_ID` | DiscordサーバーのID（スラッシュコマンドの即時反映に必要） | 必須 |
| `API_GATEWAY_URL` | サーバー操作用のAWS API Gateway URL | 必須 |
| `WHITELIST_API_URL` | ホワイトリスト操作用のAWS API Gateway URL | 必須 |
| `EBS_VOLUME_ID` | バックアップ対象のEBSボリュームID | 必須 |
| `DEATH_CHANNEL_ID` | 死亡通知を送るDiscordチャンネルID | 必須 |
| `AWS_REGION` | AWSリージョン（デフォルト: ap-northeast-1） | 任意 |
| `DEATH_WEBHOOK_PORT` | 死亡通知受信ポート（デフォルト: 8080） | 任意 |
| `DEATH_WEBHOOK_SECRET` | Webhook認証用シークレット | 任意 |
| `MC_LOG_FILE` | MinecraftのログファイルパスEC2側 | 任意 |
| `DISCORD_WEBHOOK_URL` | 死亡通知送信先のDiscord Webhook URL | 任意 |
